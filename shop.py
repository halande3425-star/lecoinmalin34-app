...
def import_export():
 if not EXPORT.exists():return {'imported':0,'categories':0,'accessible_photos':0,'source':'missing'}
 try:data=json.loads(EXPORT.read_text(encoding='utf-8'))
 except Exception:return {'imported':0,'categories':0,'accessible_photos':0,'source':'invalid'}
 messages=data.get('messages') or []; topics={}
 for m in messages:
 ...
 for m in messages:
  ...
  if any(term in name.casefold() for term in NON_PRODUCT):continue
  raw=text_value(m.get('text')).strip()
  if not(has_media(m) or raw):continue
  # Telegram album-like messages posted in the same second are kept as one card.
  stamp=m.get('date',''); key=(tid,stamp[:19] if isinstance(stamp,str) else '')
 ...
 with db() as c:
  ...
  for (tid,_),items in groups.items():
   ids=[int(x['id']) for x in items]; primary=ids[0]; raw='\n'.join(text_value(x.get('text')).strip() for x in items if text_value(x.get('text')).strip()).strip()
   title=(raw.splitlines()[0][:180] if raw else f'Publication Telegram #{primary}')
   paths=[p for x in items for p in [x.get('photo') or x.get('file') or ''] if isinstance(p,str) and p]
   local=[p for p in paths if (BASE_DIR/p).is_file()]
   if local:accessible+=1
   cat=topics.get(tid,'Sans topic'); row=c.execute('SELECT id FROM categories WHERE telegram_id=?',(tid,)).fetchone(); cid=row['id'] if row else None
   existing=c.execute('SELECT id FROM products WHERE telegram_message_id=?',(primary,)).fetchone()
   values=(title,cat,cid,local[0] if local else (paths[0] if paths else ''),'accessible' if local else ('telegram_pending' if paths else 'missing'),json.dumps(ids,ensure_ascii=False),raw)
   if existing:c.execute('UPDATE products SET title=?,category=?,category_id=?,image_path=?,media_status=?,telegram_message_ids=?,description=?,active=1 WHERE id=?',(*values,existing['id']))
   else:c.execute('INSERT INTO products(telegram_message_id,title,stock,active,category,category_id,image_path,media_status,telegram_message_ids,description) VALUES(?,?,?,?,?,?,?,?,?,?)',(primary,title,0,1,cat,cid,values[3],values[4],values[5],raw)); imported+=1
 return {'imported':imported,'total_catalog_products':len(groups),'categories':len(topics),'accessible_photos':accessible,'source':'result(2)(1).json'}
...
def login():
 ...
 with db() as c:cur=c.execute('INSERT INTO users(first_name,last_name,created_at) VALUES(?,?,?)',(first,last,datetime.utcnow().isoformat()));session['user_id']=cur.lastrowid;session['name']=f'{first} {last}'
